    ...
    var body: some View {
        NavigationView {
            VStack {
                ...
                .onAppear {
                    appState.fetchWeatherData(location: appState.locationManager.location) { weatherInfo in
                        if let weatherInfo = weatherInfo,
                           let (temp, _) = appState.parseWeatherInfo(weatherInfo: weatherInfo),
                           let hum = appState.parseHumidity(weatherInfo: weatherInfo) {
                ...
                List {
                    ForEach(appState.alarms) { alarmObject in
                        AlarmRowView(alarmObject: alarmObject)
                    }
