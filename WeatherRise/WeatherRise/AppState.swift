...
class AppState: ObservableObject {
    ...
    func fetchWeatherData(location: CLLocation?, completion: @escaping (WeatherInfo?) -> Void) {
        ...
        let url = URL(string: "https://api.openweathermap.org/data/2.5/weather?lat=\(location.coordinate.latitude)&lon=\(location.coordinate.longitude)&units=metric&appid=\(apiKey)")!
